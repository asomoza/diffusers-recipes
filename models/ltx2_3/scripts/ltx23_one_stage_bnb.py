...
Pipeline:
  ...
  Stage 1 -> Generate video+audio at full resolution with CFG (non-distilled, 40 steps)
  Decode  -> Streaming temporal decode + audio decode

Each step loads only what it needs and frees everything before the next step to minimize VRAM.
"""
...
MODEL_PATH = "OzzyGT/LTX-2.3"
BNB_4BIT_MODEL_PATH = "OzzyGT/LTX-2.3-bnb-nf4"
BNB_MODEL_PATH = BNB_4BIT_MODEL_PATH
USE_4BIT_TEXT_ENCODER = True
...
if USE_4BIT_TEXT_ENCODER:
    bnb_te_path = BNB_4BIT_MODEL_PATH
...
del embeds_pipe_kwargs, embeds_pipe
flush()
...
output_path = f"outputs/ltx23/ltx23_one_stage_bnb_4bit_{width}x{height}_{seconds}s_seed{seed}.mp4"
encode_video(
    video[0],
    fps=frame_rate,
