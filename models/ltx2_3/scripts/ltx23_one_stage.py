...
Benchmarks (960x544, 10s @ 24fps, 30 steps, RTX 5090):
  ...
  ├────────────────────────┼──────────┼───────────┼──────────┤
  │ Encode prompts         │     9.5s │   4.61 GB │ 24.85 GB │
  │ Stage 1 (960x544)      │  11m 08s │  10.93 GB │  2.30 GB │
  │ Decode (streaming)     │     8.1s │   1.44 GB │  4.98 GB │
  │ TOTAL                  │  11m 29s │  10.93 GB │ 24.85 GB │
  └────────────────────────┴──────────┴───────────┴──────────┘

  Note: The 24.85 GB RAM peak is transient — it occurs while loading the Gemma3 12B
...
output_path = f"outputs/ltx23/ltx23_one_stage_{width}x{height}_{seconds}s_seed_{seed}.mp4"
encode_video(
    video[0],
    fps=frame_rate,
