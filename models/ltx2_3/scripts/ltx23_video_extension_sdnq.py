...
from pipeline_ltx2_multimodal import (
    LTX2AudioCondition,
    LTX2MultiModalPipeline,
    LTX2VideoCondition,
    load_audio,
    load_video,
)
...
SOURCE_NUM_FRAMES = round(SOURCE_SECONDS * FRAME_RATE) // 8 * 8 + 1
...
The capybara pauses, then a small smile lifts the corners of its stitched mouth. It glances toward the river and back to the camera, and with a warmer, amused tone adds: "actually... I think this one is video extension." A felt butterfly drifts gently across the frame, its tiny wings flapping in slow stitched motion, while the camera continues its gentle push-in, settling on a soft close-up of the capybara's face as it lets out a quiet, contented sigh.
...
# Lock the first SOURCE_SECONDS of audio to the source clip; the model generates audio for the rest.
waveform = load_audio(AUDIO_PATH, target_sample_rate=pipe.audio_vae.config.sample_rate, seconds=SOURCE_SECONDS)
audio_conditions = [LTX2AudioCondition(audio=waveform, start_time=0.0, end_time=float(SOURCE_SECONDS), strength=1.0)]

# Source video frames anchored at latent index 0 — the first SOURCE_SECONDS of the output are
# locked to this clip, and the remaining frames are generated as a continuation.
source_frames = load_video(SOURCE_VIDEO_PATH)[:SOURCE_NUM_FRAMES]
video_conditions = [LTX2VideoCondition(frames=source_frames, index=0, strength=1.0)]
...
result = pipe(
    ...
    # Source video locked at the start of the output timeline (extension)
    video_conditions=video_conditions,
    # Source audio locked for the first SOURCE_SECONDS of the output timeline
...
OUTPUT_PATH = f"outputs/ltx23/ltx2_video_extension_{WIDTH}x{HEIGHT}_{SECONDS}s_seed{SEED}.mp4"
